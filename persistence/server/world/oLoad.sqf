...
private ["_strToSide", "_maxLifetime", "_isWarchestEntry", "_isBeaconEntry", "_isCameraEntry", "_isBoomerangEntry", "_worldDir", "_methodDir", "_objCount", "_objects", "_exclObjectIDs"];
...
_isBoomerangEntry = { [_variables, "has_boomerang", false] call fn_getFromPairs };
...

{
	...
	if (!isNil "_class" && !isNil "_pos" && {_maxLifetime <= 0 || _hoursAlive < _maxLifetime}) then
	{
		...
		_allowed = switch (true) do
		{
			...
			case (call _isCameraEntry):         { _cctvCameraSavingOn };
			case (call _isBoomerangEntry):      { _boomerangSavingOn };
		...
		// Boomerang
		if (isNil "_vehicle_has_station" || {typeName _vehicle_has_station != typeName []}) then {
			has_boomerang = [];
			};
			
			 if (_obj getVariable ["has_boomerang",false]) then {
				cctv_cameras pushBack _obj;
				publicVariable "_vehicle_has_station";
		};
